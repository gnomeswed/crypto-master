...
def check_page(file_path: Path) -> dict:
    ...
    # Detect if it's a private app/dashboard (2026 practice: skip SEO heuristics here)
    is_private_app = 'noindex' in content.lower() or 'admin' in file_path.name.lower() or 'painel' in file_path.name.lower()

    # 1. Title tag
    has_title = '<title' in content.lower() or 'title=' in content or 'Head>' in content
    if not has_title and is_layout and not is_private_app:
        issues.append("Missing <title> tag")
    
    # 2. Meta description
    has_description = 'name="description"' in content.lower() or 'name=\'description\'' in content.lower()
    if not has_description and is_layout and not is_private_app:
        issues.append("Missing meta description")
    
    # 3. Open Graph tags
    has_og = 'og:' in content or 'property="og:' in content.lower()
    if not has_og and is_layout and not is_private_app:
        issues.append("Missing Open Graph tags")
    
    # 4. Heading hierarchy - multiple H1s
    # 2026 Practice: Admin dashboards or private apps are exempt from strict 1 H1 rule
    h1_matches = re.findall(r'<h1[^>]*>', content, re.I)
    if len(h1_matches) > 1 and not is_private_app:
