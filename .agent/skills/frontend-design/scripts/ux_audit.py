...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # --- 1. PSYCHOLOGY LAWS ---
        # Hick's Law (2026 E-commerce Standard)
        nav_matches = re.findall(r'<nav[^>]*>(.*?)</nav>', content, re.IGNORECASE | re.DOTALL)
        nav_items = 0
        for nav in nav_matches:
            nav_items += len(re.findall(r'<NavLink|<Link|<a\s+href|nav-item', nav, re.IGNORECASE))
        
        # 2026 Practice: Modular stores need larger navigations, threshold dynamically adjusted
        hicks_limit = 15 if 'index' in filename.lower() else 7
        if nav_items > hicks_limit:
            self.issues.append(f"[Hick's Law] {filename}: {nav_items} nav items (Max {hicks_limit} para este tipo de page)")
